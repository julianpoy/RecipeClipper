// Self import for mock
import * as self from './ml.js';
import {
  applyLIBlockStyling
} from './element';
import {
  ingredientSectionHeader,
  instructionSectionHeader,
} from '../constants/regex.js';

export const getDocumentContent = () => {
  applyLIBlockStyling(document.body);
  const pageText = document.body.innerText;

  return pageText.split('\n').map(line => line.trim()).filter(line => line);
};

export const findPotentialSetsByHeader = (headerRegexp) => {
  const content = self.getDocumentContent();

  return content.filter(line => line.match(headerRegexp))
    .map(line => content.slice(content.indexOf(line) + 1));
}

export const loadModel = async () => {
  const modelUrl = window.RC_ML_MODEL_ENDPOINT;
  if (!modelUrl) throw new Error("You must provide RC_ML_MODEL_ENDPOINT on the window to use local classification");
  return await window.tf.loadLayersModel(modelUrl);
}

export const mlClassifyLocal = async (lines) => {
  const model = await self.loadModel();
  const useModel = await window.use.load();

  const predictions = [];
  for (var i = 0; i < lines.length; i++) {
    const encodedData = await useModel.embed([lines[i]]);
    const prediction = model.predict(encodedData).dataSync();
    predictions.push(prediction);
  }

  return predictions;
}

export const mlClassifyRemote = async (lines) => {
  const remote = window.RC_ML_CLASSIFY_ENDPOINT;
  if (!remote) throw new Error("You must provide RC_ML_CLASSIFY_ENDPOINT on the window to use remote classification");

  const response = await fetch(remote, {
    method: 'POST',
    body: JSON.stringify({
      sentences: lines
    }),
    headers: {
      'Content-Type': 'application/json'
    },
  });

  return response.json();
}

export const mlClassify = async (lines) => {
  const isTFJSAvailable = window.tf && window.tf.loadLayersModel;
  const isUSEAvailable = window.use && window.use.load;

  if (isTFJSAvailable && isUSEAvailable) return self.mlClassifyLocal(lines);
  else return self.mlClassifyRemote(lines);
};

export const mlFilter = async (lines, type) => {
  const predictions = await self.mlClassify(lines);

  let lastType = -1;
  const filteredOutput = [];
  for (var i = 0; i < lines.length; i++) {
    const predictedType = predictions[i].indexOf(Math.max(...predictions[i])) + 1;

    // Allow one line of error to be included (say, an ingredient header or a stray bit of formatting)
    // Also allow some non-ingredient text at the very start (say, a section header)
    if (predictedType === type || lastType === type || i === 0) {
      filteredOutput.push(lines[i]);
      lastType = predictedType;
    } else {
      break;
    }
  }

  // Remove last element if it is not of the desired type (consider our allowed buffer above may insert one undesired item at end)
  if (filteredOutput.length > 0 && lastType !== type) {
    filteredOutput.pop();
  }

  return filteredOutput;
}

export const findFullSearch = async (type) => {
  const content = self.getDocumentContent();

  const predictions = await self.mlClassify(content);

  const { groups, workingGroup } = content.reduce((acc, line, idx) => {
    const predictedType = predictions[idx].indexOf(Math.max(...predictions[idx])) + 1;

    const correctType = predictedType === type;
    const lastType = acc.workingGroup && acc.workingGroup.length > 0 ? acc.workingGroup[acc.workingGroup.length - 1].type : -1;

    // Allow one line of error to be included in group (say, an ingredient header or a stray bit of formatting)
    // Also allow some non-ingredient text at the very start (say, a section header)
    if (predictedType === type || lastType === type || idx === 0) {
      acc.workingGroup = acc.workingGroup || [];
      acc.workingGroup.push({
        text: line,
        type: predictedType
      });
    } else {
      // Group is considered complete
      if (acc.workingGroup && acc.workingGroup.length > 0) acc.groups.push(acc.workingGroup);
      acc.workingGroup = null;
    }
    
    return acc;
  }, {
    workingGroup: null,
    groups: [],
  });

  // If leftover working group (if ingredients/instructions end at very end of page)
  if (workingGroup) groups.push(workingGroup);

  groups.forEach(group => {
    // Remove last element if it is not of the desired type (consider our allowed buffer above may insert one undesired item at end)
    if (group.length > 0 && group[group.length - 1].type !== type) {
      group.pop();
    }
  });

  return groups
    .map(group => group.map(item => item.text))
    .map(group => group.join('\n'))
    .reduce((a, b) => a.length > b.length ? a : b, "");
}


export const findByHeader = async (type) => {
  const headerRegexp = type === 1 ? ingredientSectionHeader : instructionSectionHeader;
  const potentialSets = self.findPotentialSetsByHeader(headerRegexp);

  const sets = [];
  for (const potentialSet of potentialSets) {
    const set = await self.mlFilter(potentialSet, type);
    sets.push(set);
  }

  return sets.map(set => set.join('\n'))
    .reduce((a, b) => a.length > b.length ? a : b, "");
}

// Type 1 for ingredients
// Type 2 for instructions
// Others to be implemented in future...
export const grabByMl = async (type) => {
  if (window.RC_ML_DISABLE) return "";

  return await self.findByHeader(type) || await self.findFullSearch(type);
}
